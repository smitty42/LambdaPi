...
class PycloudMiddleman(tornado.web.RequestHandler):
    def get(s):
        ...
        nodes = map(lambda x: "%03d" % x,xrange(number_of_nodes))
        ...
        first_occurrences = filter(lambda x: x != -1, first_occurrences_in_nodes)
        if first_occurrences:
            first_occurrence_node = first_occurrences_in_nodes.index(next( i for i in first_occurrences_in_nodes if i != -1))
            first_occurrence = first_occurrences[0] + (characters_in_node * (first_occurrence_node + 1))
        ...
        data_dict['first_occurrence'] = first_occurrence
...
def pi_string_search(data):
    ...
    first_occurrence = pi.find(sub)
    end = time.time()
    delta = end - start
    return [delta, instances, node, first_occurrence]
